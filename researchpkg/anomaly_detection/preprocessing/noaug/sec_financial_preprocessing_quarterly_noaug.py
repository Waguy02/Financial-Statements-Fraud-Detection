...
COL_N_FEATURES = "n_noaug_features"
...
NOAUG_FEATURES_DESCRIPTION_DICT = {
    # Important Tags
...
NOAUG_FEATURES_SHORT_DESCRIPTION_DICT = {
    key: value.split(":")[0] for key, value in NOAUG_FEATURES_DESCRIPTION_DICT.items()
}

MAX_MISSING_RATIO = 3.1
...
def extract_financial_data(root_dir: Path, start_year: int, end_year: int) -> None:
    ...
    if not (FINANCIALS_DIR_NOAUG).exists():
        (FINANCIALS_DIR_NOAUG).mkdir(parents=True)
    tags_index_csvfile = FINANCIALS_DIR_NOAUG / "sec_tags_index.csv"
    tags_index_csvfile_versionned = (
        FINANCIALS_DIR_NOAUG / "sec_tags_index_versionned.csv"
    )
...
def filter_financial_data(root_dir: Path, start_year, end_year):
    all_directories = [
        directory
    ...
    assert set(IMPORTANT_TAGS).issubset(
        tags_set
    ), "Some important tags are missing from subset"
    ...
    def filter_dir(directory):
        ...
        partial_csv_file = (
            FINANCIALS_DIR_NOAUG / f"sec_financials_quarterly_{year_quarter_name}.csv"
        )

        partial_df = pd.read_csv(partial_csv_file)
        partial_df = pivot_financial_data(partial_df)
        partial_df.fillna(0.0, inplace=True)
        ...
        partial_df = partial_df[
            partial_df[IMPORTANT_TAGS].ne(0).sum(axis=1)
            > len(IMPORTANT_TAGS) / MAX_MISSING_RATIO
        ]
        # Logging average number of missing importants
        final_size = len(partial_df)

        if len(partial_df) > 0:
            logging.info(f"Filter {year_quarter_name}. {initial_size=}, {final_size=}")
    ...
    processed_data_list = Parallel(n_jobs=njobs)(
        delayed(filter_dir)(directory)
        for directory in tqdm.tqdm(all_directories, "Filter directories")
    )
...
def merge_financial_data(root_dir: Path, start_year, end_year):
    ...
    dataset_csvfile = FINANCIALS_DIR_NOAUG / "sec_financials_quarterly.csv"
    ...
    def merge_single_directory(directory):
        ...
        # Update the quarter and year in prev_partial_df to match what we expect
        prev_partial_df[COL_YEAR] = year
        prev_partial_df[COL_QUARTER] = f"q{quarter}"
        ...
        partial_df = pd.merge(
            partial_df,
            prev_partial_df,
            on=[COL_COMPANY, COL_CIK, COL_YEAR, COL_QUARTER, COL_SICAGG, COL_SIC],
        )
        ...
        partial_df[COL_N_TAGS_TOTAL] = partial_df[COL_N_TAGS]
...
def pivot_financial_data(data: pd.DataFrame) -> pd.DataFrame:
    pivoted_df = data.pivot_table(
        index=["company", "cik", "year", "quarter", "sicagg", "sic"],
        columns="tag",
        values="value",
        aggfunc="first",
    ).reset_index()
...
def compute_beineish_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    def calculate_beneish_features(row):
        ...
        curr_gross_margin = safe_divide(
            get_tag_value(REVENUES_TAG, row) - get_tag_value(COST_OF_REVENUE_TAG, row),
            get_tag_value(REVENUES_TAG, row),
        )
...
def save_dataset_stats():
    ...
    stats_dict = {
        ...
        "n_noaug_features_stats": get_col_stat(COL_N_FEATURES),
        "max_missing_ratio": MAX_MISSING_RATIO,
    }
    save_dataset_config(str(FINANCIALS_DIR_NOAUG), **stats_dict)

    main_df.to_csv(FINANCIALS_DIR_NOAUG / "sec_financials_quarterly_no_financials.csv")
...
if __name__ == "__main__":
    ...
    filter_financial_data(Path(SEC_FINANCIALS_RAW_DATASET_PATH), start_year, end_year)
